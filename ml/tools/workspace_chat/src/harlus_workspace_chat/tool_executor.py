...
class ToolExecutorNode:
    ...
    async def _process_doc_search_tool_call(
        self, tool_call
    ) -> tuple[list[DocSearchRetrievedNode], ToolMessage]:
        retrieved_nodes = []
        ...
        nodes_with_scores = get_nodes_with_score_from_doc_search_tool_result(
            tool_result, tool_type
        )
        for node_with_score in nodes_with_scores:
            retrieved_node = get_doc_search_retrieved_node_from_node_with_score(
                node_with_score, self.file_id_to_path
            )
    ...
    async def _process_tavily_search_tool_call(
        self, tool_call
    ) -> tuple[list[TavilyToolRetrievedWebsite], ToolMessage]:
        tool = self.tools_by_name[tool_call["name"]]
        tool_metadata = self.tool_name_to_metadata[tool_call["name"]]
        tool_friendly_name = tool_metadata.get("friendly_name", "")
        writer = get_stream_writer()
        ...
        for result in tool_result.get("results", []):
            tavily_tool_retrieved_website = (
                get_tavily_tool_retrieved_website_from_tool_result(result)
            )
    ...
    async def __call__(self, state: dict) -> AsyncIterator[dict | AIMessageChunk]:
        ...
        for result in results:
            retrieved_data, tool_message = result
            if not isinstance(retrieved_data, list):
                continue
            retrieved_data_is_nodes = all(
                isinstance(data, DocSearchRetrievedNode) for data in retrieved_data
            )
            retrieved_data_is_websites = all(
                isinstance(data, TavilyToolRetrievedWebsite) for data in retrieved_data
            )
            if retrieved_data_is_nodes or retrieved_data_is_websites:
                tool_messages.append(tool_message)
                retrieved_items.extend(retrieved_data)
