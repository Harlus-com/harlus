...
<<<<<<< HEAD
    ...
    def update_tools(
        self,
        internal_tools: list[DocSearchToolWrapper],
        external_tools: list[DocSearchToolWrapper],
    ):
=======
...
<<<<<<< HEAD
        get_tree_tool_node = ToolExecutorNode(
            tools=self.tools["internal"]["doc_search_summary"],
            tool_name_to_metadata=self.tool_name_to_metadata["internal"][
                "doc_search_summary"
            ],
            message_state_key="internal_messages",
            retrieved_items_state_key="internal_retrieved_items",
        )
        refine_tree_tool_node = ToolExecutorNode(
            tools=self.tools["internal"]["doc_search_retriever"],
            tool_name_to_metadata=self.tool_name_to_metadata["internal"][
                "doc_search_retriever"
            ],
            message_state_key="internal_messages",
            retrieved_items_state_key="internal_retrieved_items",
        )
        add_statement_source_texts_tool_node = ToolExecutorNode(
            tools=self.tools["internal"]["doc_search_retriever"],
            tool_name_to_metadata=self.tool_name_to_metadata["internal"][
                "doc_search_retriever"
            ],
        ...
        verify_tree_tool_node = ToolExecutorNode(
            tools=self.tools["external"]["doc_search_retriever"],
            tool_name_to_metadata=self.tool_name_to_metadata["external"][
                "doc_search_retriever"
            ],
            message_state_key="external_messages",
            retrieved_items_state_key="external_retrieved_items",
        )
...
<<<<<<< HEAD
        self.get_tree_llm = self.LLM.bind_tools(
            self.tools["internal"]["doc_search_summary"]
        )
        self.refine_tree_llm = self.LLM.bind_tools(
            self.tools["internal"]["doc_search_retriever"]
        )
        self.verify_tree_llm = self.LLM.bind_tools(
...
>>>>>>> e7d087e (progress update)
    ...
    async def _get_tree(self, state: ContrastToolGraphState) -> AsyncIterator[dict]:
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "prompts/get_tree_prompt.md"), "r"
        ) as f:
            system_prompt = f.read()
<<<<<<< HEAD

        system_prompt = (
            system_prompt + self.tools_descriptions["internal"]["doc_search_summary"]
        )
=======
...
<<<<<<< HEAD
    ...
    async def _add_statement_source_texts(
        self, state: ContrastToolGraphState
    ) -> AsyncIterator[dict]:
        ...
        with open(
            os.path.join(
                ...
                "prompts/add_statement_source_texts_prompt.md",
            ),
            "r",
        ) as f:
            system_prompt = f.read()
        ...
        system_prompt = (
            system_prompt + self.tools_descriptions["internal"]["doc_search_retriever"]
        )
        prompt = [
            SystemMessage(content=system_prompt),
            *state["internal_messages"],
...
>>>>>>> e7d087e (progress update)
    ...
    async def _refine_tree(self, state: ContrastToolGraphState) -> AsyncIterator[dict]:
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "prompts/refine_tree_prompt.md"),
            "r",
        ) as f:
            system_prompt = f.read()
<<<<<<< HEAD
...
>>>>>>> e7d087e (progress update)
    ...
    async def _verify_tree(self, state: ContrastToolGraphState) -> AsyncIterator[dict]:
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "prompts/verify_tree_prompt.md"),
            "r",
        ) as f:
            system_prompt = f.read()
<<<<<<< HEAD
        system_prompt = (
            system_prompt + self.tools_descriptions["external"]["doc_search_retriever"]
        )
=======
...
<<<<<<< HEAD
    async def _format_output(
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "prompts/format_output_prompt.md"),
            "r",
        ) as f:
            system_prompt = f.read()
...
>>>>>>> e7d087e (progress update)
        ...
        yield {"claim_comments": claim_comments}
...
<<<<<<< HEAD
        graph_builder.add_node(
            "get_tree", self._get_tree, metadata={"name": "get_tree"}
        )
        graph_builder.add_node(
            "refine_tree", self._refine_tree, metadata={"name": "refine_tree"}
        )
        graph_builder.add_node(
            "verify_tree", self._verify_tree, metadata={"name": "verify_tree"}
        )
        graph_builder.add_node(
            "format_output", self._format_output, metadata={"name": "format_output"}
        )
        graph_builder.add_node(
            "add_statement_source_texts",
            self._add_statement_source_texts,
            metadata={"name": "add_statement_source_texts"},
        )
        graph_builder.add_node(
            "get_source_nodes",
            self._get_source_nodes,
            metadata={"name": "get_source_nodes"},
        )
        graph_builder.add_node(
            "tools_get_tree",
            self.tool_nodes["get_tree"],
            metadata={"name": "tools_get_tree"},
        )
        graph_builder.add_node(
            "tools_verify_tree",
            self.tool_nodes["verify_tree"],
            metadata={"name": "tools_verify_tree"},
        )
        graph_builder.add_node(
            "tools_refine_tree",
            self.tool_nodes["refine_tree"],
            metadata={"name": "tools_refine_tree"},
        )
        graph_builder.add_node(
            "tools_add_statement_source_texts",
            self.tool_nodes["add_statement_source_texts"],
            metadata={"name": "tools_add_statement_source_texts"},
        )
...
<<<<<<< HEAD
        ...
        graph_builder.add_edge(
            "tools_add_statement_source_texts", "add_statement_source_texts"
        )
=======
        #graph_builder.add_conditional_edges(
        #    "add_statement_source_texts",
