...
def package_tool(tool_name):
    ...
    for file in os.listdir(os.path.join(tool_dir, "dist")):
        if file.endswith(".whl"):
            wheel_paths.append(os.path.join(tool_dir, "dist", file))
    if len(wheel_paths) == 0:
        raise Exception("No wheel found in dist directory")
    if len(wheel_paths) > 1:
        raise Exception("Multiple wheels found in dist directory")
...
def link(tool_name):
    if get_dep(tool_name) is not None:
        subprocess.run(["pip", "uninstall", "-y", tool_name])
    local_tool_path = os.path.abspath(f"../ml/tools/{tool_name}")
...
def unlink(tool_name):
    ...
    print(f"Installe {highest_version_wheel[1]}")
...
if __name__ == "__main__":
    ...
    p.add_argument("tool_name", help="name of your tool directory")
