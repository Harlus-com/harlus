...
export const ChatHistory: React.FC<ChatHistoryProps> = ({
  ...
  const [isThreadSelectorOpen, setIsThreadSelectorOpen] = useState(true);
  ...
  return (
    <div className="border-b border-gray-100">
      <button
        onClick={() => setIsThreadSelectorOpen(!isThreadSelectorOpen)}
        className="w-full px-3.5 py-2 flex items-center justify-between hover:bg-gray-50 transition-colors"
      >
        <div className="flex items-center gap-2">
          <BookOpen className="w-4 h-4 text-gray-400" />
          <span className="text-sm text-gray-700">
            {threads.length === 0 ? "No chat history" : "Chat history"}
          </span>
        </div>
        <ChevronDown
          className={cn(
            "w-4 h-4 text-gray-400 transition-transform",
            isThreadSelectorOpen ? "transform rotate-180" : ""
          )}
        />
      </button>
