...
const ContrastAnalysisPanel: React.FC<ContrastAnalysisProps> = ({ files, isOpen, onClose }) => {
  const [selectedFile1, setSelectedFile1] = useState<WorkspaceFile | null>(null);
  const [selectedFile2, setSelectedFile2] = useState<WorkspaceFile | null>(null);
  ...
  const handleRunAnalysis = async () => {
    ...
    try {
      const result = await fileService.runContrastAnalysis(selectedFile1.id, selectedFile2.id);
          ...
          <>
            <div className="mb-6">
              ...
              <p className="text-muted-foreground mb-4">
                Select two documents to analyze and compare their content, structure, and key points.
              </p>
                  ...
                  <div className="border rounded-md overflow-hidden">
                    <div className="max-h-[200px] overflow-y-auto">
                      {files.map(file => (
                        <div
                          key={file.id}
                          className={`p-3 cursor-pointer hover:bg-muted ${selectedFile1?.id === file.id ? 'bg-muted' : ''}`}
                  ...
                  <div className="border rounded-md overflow-hidden">
                    <div className="max-h-[200px] overflow-y-auto">
                      {files.map(file => (
                        <div
                          key={file.id}
                          className={`p-3 cursor-pointer hover:bg-muted ${selectedFile2?.id === file.id ? 'bg-muted' : ''}`}
          ...
          <div>
            ...
            <div className="mb-6">
              ...
              <div className="flex items-center justify-between bg-muted p-3 rounded-md">
                <div className="truncate max-w-[45%]">{selectedFile1?.name}</div>
                <ArrowRight size={16} className="text-muted-foreground mx-2" />
                <div className="truncate max-w-[45%]">{selectedFile2?.name}</div>
              </div>
            </div>
