...
export default function RefreshDataDialog({
  ...
  const [dialogInputDestination, setDialogInputDestination] =
    useState<string>("");
  const [dialogInputStartDate, setDialogInputStartDate] =
  ...
  const performRefresh = useCallback(
    async (destination: string, startDate: string) => {
      ...
      try {
        ...
        toast.error(
          `Error refreshing online data: ${error.message || "Unknown error"}`
        );
        setIsRefreshSuccess(false);
      } finally {
    ...
    [isRefreshingOnlineData, workspace]
  );
        ...
        <AlertDialogContent className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 max-w-md w-full">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-gray-600 dark:text-gray-300 mt-2">
              Please enter the folder path where you want to download the online
              data, and select the start date for the data pull.
            </AlertDialogDescription>
          </AlertDialogHeader>
